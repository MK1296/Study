...
-- 5. 店舗コード（'EA03'）の店舗に関連する在庫情報（商品コード、在庫数、商品名）を取得して下さい。
    SELECT st.product_code,st.quantity,g.product_name
    FROM stock_table st
    INNER JOIN goods_table g
        ON st.product_code = g.product_code
...
-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、価格=4500円、更新日付=本日日付」のデータを追加して下さい。。※実行後の「データ出力」画面も貼付すること。
    INSERT INTO goods_table (product_code, product_name, price, update_day)
    VALUES ('M001', 'マフラー', 4500, CURRENT_DATE);

    SELECT * FROM goods_table
    WHERE product_code = 'M001';
...
-- 9. 在庫テーブルの商品（'S987'）かつ、店舗（'EA01'）の在庫数を10増やす更新を行って下さい。※実行後のSELECT結果も貼付すること。
    ...
    WHERE product_code = 'S987' AND store_code = 'EA01';
...
-- 10. 商品コード（'Z939'）かつ、店舗コード（'EA04'）に関連する在庫情報を在庫テーブルから削除して下さい。※実行後の「データ出力」画面も貼付すること。
    ...
    WHERE product_code = 'Z939' AND store_code = 'EA04';
